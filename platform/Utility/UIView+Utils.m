...
- (void)setLeftTopPosition:(CGPoint)leftTopPosition {
	self.left = leftTopPosition.x;
	self.top = leftTopPosition.y;
}
...
- (void)rotationAnimationWithPerCycleDuration:(double)duration
    ...
    rotationAnimation.toValue = [NSNumber numberWithFloat:clockwise ? rotation : -rotation];
...
static char const *const AnimationCompletionKey = "AnimationCompletionKey";
- (void)setAnimationCompletion:(void (^)(BOOL))animationCompletion {
    objc_setAssociatedObject(self, AnimationCompletionKey, animationCompletion, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void (^)(BOOL))animationCompletion {
    return objc_getAssociatedObject(self, AnimationCompletionKey);
}
